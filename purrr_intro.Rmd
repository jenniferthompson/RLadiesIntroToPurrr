...
author: "Jennifer Thompson, R-Ladies November 2017"
output:
  html_notebook:
    theme: journal
    highlight: kate
...
Looks good. But what we *really* want is one single data.frame for recreation visits with all the years included, and a variable that indicates the year. We can do that in ONE STEP with `purrr::map_df`!

Remember, `map_df` knows that we want to iterate over something and get a data.frame as our final result. So here, we'll iterate over year, get a data.frame for each one, and bind all the rows together into a final data.frame. (You may have done something similar before, with `do.call(rbind, lapply(...))`, or `dplyr::bind_rows(list(...))`.)

We can do this for recreational visits only using an anonymous function.

```{r download_recvisits}
rec_visits <- map_df(
  ## Iterate over years 2007-2016, which are part of our file name
  .x = 2007:2016,
  ## What do we want to *do* with each year? -> Define our function
  ## ~ : starts an anonymous function
  ## . : says "this is where the thing I'm iterating over goes"
  .f = ~ data.world::query(
    data.world::qry_sql(sprintf("SELECT * FROM recreation_visits_%s", .)),
    dataset = url_recvisits
  ),
  ## Add an indexing column to each dataset, so we can tell each year apart
  .id = "year"
)
...
LOOKING GOOD. Notice that the "year" column is a) a character and b) 1-9, not 2007-2016.

HOWEVER. We also want the number of backcountry and tent campers. Those datasets look exactly like the total recreation visit data, so we want to do basically the same thing - which means in retrospect, we should have written our own "user-defined" function, because [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) (Don't Repeat Yourself). So let's do that now.
...
download_nps_year <- function(
  ...
  data.world::query(
    data.world::qry_sql(sprintf("SELECT * FROM %s_%s", table_prefix, year)),
    dataset = url
  ) %>%
    ## As long as we're writing a custom function, let's...
    ## Take only the variables we want
...
Note that the code looks a lot cleaner; anonymous functions are great for short functions that you don't want to reuse, or if you're new to function writing and want to start out small, but writing your own functions has a lot of benefits.
